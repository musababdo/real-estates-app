...
class _AddAqarState extends State<AddAqar> {
  ...
  File _image,imageSearch;
  String _chosenValue,specilValue,locationValue,searchValue,id;
  int chosenint,locationint,searchint;
  bool visible = false;
  ...
  Future saveAqar() async{
    try{
      List<int> imageBytes = _image.readAsBytesSync();
      String baseimage = base64Encode(imageBytes);
      var url = Uri.parse('http://10.0.2.2/aqar/save_aqar.php');
                      ...
                      children: <Widget>[
                        ...
                        Padding(
                          ...
                          child: new TextFormField(
                            ...
                            validator:(value) {
                              if (value.isEmpty) {
                                return _errorMessage("الموقع");
                                // ignore: missing_return
                              }
                            },
                            controller: location,
                            decoration: InputDecoration(
                                labelText: 'ادخل الموقع',
                                hintText: "الموقع"
                            ),
                          ),
                        ),
                        ...
                        if (_globalKey.currentState.validate()){
                          ...
                          try{
                            if(searchValue == null){
                              ...
                              if(searchint == 2){
                                Fluttertoast.showToast(
                                    msg: "عذرا يجب عليك عمل شهاده بحث اولا",
                                    toastLength: Toast.LENGTH_SHORT,
                                    gravity: ToastGravity.CENTER,
                                    timeInSecForIosWeb: 1,
  ...
  Future<void> _searchoptionsDialogBox() {
    return showDialog(context: context,
        barrierDismissible: false,
        builder: (BuildContext context) {
          return AlertDialog(
  ...
  void searchopenGallery()async{
    PickedFile gallery = await _picker.getImage(source: ImageSource.gallery);
    final File file_gallery = File(gallery.path);
    setState(() {
      Navigator.pop(context);
      imageSearch=file_gallery;
    });
  }
  void searchopenCamera()async{
    PickedFile camera = await _picker.getImage(source: ImageSource.camera);
    final File file_camera = File(camera.path);
    setState(() {
      Navigator.pop(context);
      imageSearch=file_camera;
    });
  }
  locationDialog(){
    showDialog(context: context,
        builder: (BuildContext context){
          return AlertDialog(
            ...
            content:MyDialog(mlocation: location,),
          );
        }
    );
  }
